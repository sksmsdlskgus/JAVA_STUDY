...
public class MemberRepository {
    ...
    public MemberRepository() { // 회원 3명 담기
        ...
        ArrayList<Member> saveMembers = new ArrayList<>(); //초기 회원 세명만 담는용도의 ArrayList(지역변수) 여기서만 잠깐 쓰는 용도로 List하나 더 생성

        /* 설명. main을 실행할 때마다 덮어씌우는 문제를 방지하고자 파일이 없을 때만 초기 세팅이 되도록 조건문 작성 */
        if (!file.exists()) {
            saveMembers.add(new Member(1, "user01", "pass01", 20, new String[]{"발레", "수영"}, BloodType.A));
            saveMembers.add(new Member(2, "user02", "pass02", 10, new String[]{"게임", "영화시청"}, BloodType.B));
            saveMembers.add(new Member(3, "user03", "pass03", 15, new String[]{"음악감상", "독서", "명상"}, BloodType.O));


        /* 설명. 초기 멤버 3명을 파일로 객체 출력하는 코드 작성 */
        saveDefaultMembers(file, saveMembers);
        } //꺼도 데이터 저장
...
//        }
    ...
    private static void saveDefaultMembers(File file, ArrayList<Member> saveMembers) {
        ObjectOutputStream oos = null;
        try {
            oos = new ObjectOutputStream(new BufferedOutputStream(new FileOutputStream(file)));

            /* 설명. 초기 회원 3명을 각각 객체 출력 내보내기 */
            for (Member member : saveMembers) { // 한명씩 꺼내
